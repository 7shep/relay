...
export default function WeatherPanel({ index, weather, weatherStatus }) {
  ...
  return <Panel path="~/weather.now" index={index} className="weather-panel" meta={<span>{weather.location} &#8226; {weatherStatus}</span>}>
    <div className="weather-details"><div><strong className="large-temp">{weather.temp}&deg;C</strong><p>feels {weather.feelsLike}&deg; &#8226; {weather.condition.toLowerCase()}</p></div><dl><div><dt><Icon name="highLow" size={13} /></dt><dd><b>{weather.high}&deg;</b> / {weather.low}&deg;</dd></div><div><dt><Icon name="wind" size={13} /></dt><dd>{weather.windKmh} km/h</dd></div><div><dt><Icon name="humidity" size={13} /></dt><dd>{weather.humidity}%</dd></div></dl></div>
