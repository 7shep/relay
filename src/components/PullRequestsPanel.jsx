...
function GitHubDataView({ data, username, onDisconnect }) {
  ...
  return <div className="github-data"><div className="github-toolbar"><span><span className="accent-text">â—</span> connected as {username}</span><button className="github-action" onClick={onDisconnect}>disconnect</button></div><div className="github-scroll-region"><section className="github-section"><div className="github-section-header"><strong>1 Â· my open PRs</strong><span>{visibleMine.length}</span></div>{visibleMine.length ? <div className="github-pr-list">{visibleMine.map((pr) => <GitHubPRRow key={pr.id} pr={pr} showRepo onHide={hidePullRequest} />)}</div> : <p className="github-empty">No visible open PRs authored by {username}.</p>}</section><section className="github-section"><div className="github-section-header"><strong>2 Â· repos owned by me</strong><span>{visibleRepositories.length}</span></div>{visibleRepositories.length ? <div className="github-repository-list">{visibleRepositories.map((repository) => repository.pullRequests.length > 5 ? <GitHubRepositorySummary key={repository.name} repository={repository} /> : <div className="github-repository-group" key={repository.name}><div className="github-repository-title"><strong>{repository.name}</strong><span>{repository.pullRequests.length} open</span></div>{repository.pullRequests.map((pr) => <GitHubPRRow key={pr.id} pr={pr} showAuthor onHide={hidePullRequest} />)}</div>)}</div> : <p className="github-empty">No visible owned repositories with open PRs.</p>}</section></div></div>
}

function GitHubPRRow({ pr, showRepo, showAuthor, onHide }) {
  return <div className="github-pr-row"><a className="github-pr-link" href={pr.url} target="_blank" rel="noreferrer"><span className={`github-pr-icon ${pr.draft ? 'muted-text' : 'accent-text'}`} aria-hidden="true">â™§</span><span className="github-pr-copy"><strong>{pr.title}{pr.draft && <small className="draft-label">draft</small>}</strong><small>{showRepo ? `${pr.repo} #${pr.number}` : `#${pr.number}`}{showAuthor ? ` Â· opened by ${pr.author}` : ''}{pr.branch ? ` Â· ${pr.branch}` : ''}</small></span><span className="github-pr-updated">{relativeUpdated(pr.updatedAt)}</span><span className="github-external" aria-hidden="true">â†—</span></a><button type="button" className="github-pr-hide" onClick={() => onHide(pr.id)} aria-label={`Hide ${pr.title}`} title="Hide pull request">Ã—</button></div>
}

function GitHubRepositorySummary({ repository }) {
  return <a className="github-repository-summary" href={`https://github.com/${repository.name}/pulls`} target="_blank" rel="noreferrer"><span className="github-pr-icon accent-text" aria-hidden="true">â™§</span><span><strong>{repository.name}</strong><small>{repository.pullRequests.length} open PRs Â· showing repository instead</small></span><span className="github-external" aria-hidden="true">â†—</span></a>
}
